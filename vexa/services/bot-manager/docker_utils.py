...
async def start_bot_container(
    ...
    gladia_api_key_for_bot = os.getenv('GLADIA_API_KEY')

    if not gladia_api_key_for_bot:
        logger.error("CRITICAL: GLADIA_API_KEY is not set in bot-manager's environment. This should be fixed in docker-compose.yml for bot-manager service.")
        return None, None

    logger.info(f"Passing GLADIA_API_KEY to bot")
