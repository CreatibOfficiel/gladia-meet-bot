...
# --- ADDED: Pydantic Model for Gladia Session ID ---
class GladiaSessionPayload(BaseModel):
    connection_id: str = Field(..., description="The connectionId (session_uid) of the bot.")
    gladia_session_id: str = Field(..., description="The Gladia session ID for transcription retrieval.")
# --- --------------------------------------------- ---
...
@app.get("/meetings/{meeting_id}",
         ...
         description="Returns a specific meeting by its internal ID.",
         dependencies=[Depends(get_user_and_token)])
async def get_meeting_by_id(
    ...
    """Get a specific meeting by its internal ID"""
    try:
        ...
        return {
            ...
            "gladia_session_id": meeting.gladia_session_id,
...
@app.post("/bots/internal/callback/gladia-session",
          status_code=status.HTTP_200_OK,
          summary="Callback for vexa-bot to save Gladia session ID",
          include_in_schema=False) # Hidden from public API docs
async def save_gladia_session(
    payload: GladiaSessionPayload,
    db: AsyncSession = Depends(get_db)
):
    """Save the Gladia session ID for a bot connection"""
    try:
        logger.info(f"Saving Gladia session ID {payload.gladia_session_id} for connection {payload.connection_id}")
        
        # Find the meeting session by session_uid (connection_id)
        stmt = select(MeetingSession).where(MeetingSession.session_uid == payload.connection_id)
        result = await db.execute(stmt)
        meeting_session = result.scalar_one_or_none()
        
        if not meeting_session:
            logger.warning(f"No meeting session found for connection_id {payload.connection_id}")
            return {"status": "error", "message": "Meeting session not found"}
        
        # Update the meeting with the Gladia session ID
        stmt = select(Meeting).where(Meeting.id == meeting_session.meeting_id)
        result = await db.execute(stmt)
        meeting = result.scalar_one_or_none()
        
        if meeting:
            meeting.gladia_session_id = payload.gladia_session_id
            await db.commit()
            logger.info(f"Successfully saved Gladia session ID {payload.gladia_session_id} for meeting {meeting.id}")
            return {"status": "success", "meeting_id": meeting.id}
        else:
            logger.error(f"No meeting found for session {meeting_session.meeting_id}")
            return {"status": "error", "message": "Meeting not found"}
            
    except Exception as e:
        logger.error(f"Error saving Gladia session ID: {e}", exc_info=True)
        await db.rollback()
        raise HTTPException(status_code=500, detail=f"Error saving Gladia session ID: {str(e)}")
