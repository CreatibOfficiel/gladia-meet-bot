...
const waitForMeetingAdmission = async (
  ...
  try {
    log("Waiting for meeting admission (Leave button detection)...");

    // Wait for "Leave call" button which appears when bot is admitted to the meeting
    await page.waitForSelector('button[aria-label="Leave call"]', { timeout });

    log("✅ Bot admitted to meeting (Leave button detected)");
    await takeDebugScreenshot(page, "admitted-to-meeting", "leave_button_detected");

    return true;
  } catch (error: any) {
    log("❌ Failed to detect admission - Leave button not found");
    await takeDebugScreenshot(page, "admission-failed", "leave_button_timeout");
    throw new Error(
      "Bot admission failed (Leave button not found): " + error.message
    );
  }
};
